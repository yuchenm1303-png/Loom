...
Runtime v2 durably records model tool-call arguments before a tool handler runs, so ToolResult-only redaction is insufficient. Loom therefore treats every model-produced `browser_type.text` payload as transient, not just strings that happen to match a secret regex. Before the model response crosses the durable Session/event boundary, the original text is moved into an in-memory one-shot map and the persisted tool call receives only a random opaque reference. The handler resolves and consumes that reference immediately before browser execution. If the process restarts while approval is pending, the original text is intentionally unrecoverable and the action fails safely instead of loading typed data from disk.

Secret-bearing `browser_open` / `browser_navigate` URLs cannot use that typed-text channel. Common credential/token query parameters and fragments are redacted before canonical persistence, and the modified request is deliberately made schema-invalid so Loom will not navigate using a redacted credential. A future credential feature should use opaque secret handles resolved only inside the execution layer, scoped to approved domains, rather than putting raw secrets into model tool arguments.

Model-visible browser text and URLs also receive defense-in-depth redaction for common secret-shaped assignments, bearer/OpenAI/provider tokens, JWTs, and sensitive query parameters. `browser_type` never echoes typed text in its ToolResult. The browser-use DOM serializer also excludes password-field values from its LLM DOM representation, but Loom does not treat arbitrary web content as trusted: prompt injection remains untrusted input.

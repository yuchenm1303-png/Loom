...
def _rich_blocks(text: str) -> str:
    ...
    def flush_quote() -> None:
        if quote:
            out.append("<blockquote>" + "<br>".join(_inline_markup(x) for x in quote) + "</blockquote>")
            quote.clear()
    ...
    for line in text.splitlines():
        ...
        if fence:
            if in_code:
                ...
                out.append(f"<div class='codeBlock'>{badge}<pre>{html.escape(chr(10).join(code), quote=False)}</pre></div>")
                code.clear()
                language = ""
                in_code = False
            else:
...
class LoomDesktopWindow(v5.LoomDesktopWindow):
    """Desktop v6: mature visual hierarchy without changing Runtime contracts."""
    ...
    def _build_ui(self) -> None:
        ...
        if empty_body:
            empty_body.setText("Inspect, edit, run, browse, and coordinate work in this project from one durable thread.")
        ...
        self._sidebar_motion_bounds = (self.sidebar_panel.minimumWidth(), self.sidebar_panel.maximumWidth())
        self._runtime_motion_bounds = (self.activity_panel.minimumWidth(), self.activity_panel.maximumWidth())
    ...
    def _render_transcript(self) -> None:
        ...
        for message in self._durable_messages:
            role, content = v2._text(message.get("role")), v2._text(message.get("content"))
    ...
    def _message_card(role: str, content: str, *, streaming: bool) -> str:
        ...
        return f"<div class='message {role}'><div class='meta'>{label}{live}</div><div class='body'>{_rich_blocks(content)}</div></div>"

    def _render_activity(self) -> None:
        rows: list[str] = []
        for when, marker, summary in self._activity_tail[-300:]:
            tone = "good" if marker == "✓" else "bad" if marker == "!" else "accent" if marker in {"◆", "◇", "Δ"} else "muted"
            rows.append(
                f"<div class='event'><span class='marker {tone}'>{html.escape(marker)}</span>"
                f"<span class='summary'>{html.escape(summary)}</span><div class='time'>{html.escape(when or 'live')}</div></div>"
            )
        if not rows:
            rows.append("<div class='quiet'><b>Runtime is quiet</b><br><span>Model steps, tools, commands, diffs, and delegated work will appear here.</span></div>")

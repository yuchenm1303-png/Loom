...
small durable inbox entry attached to the current turn.  The runtime consumes
that inbox only at safe boundaries: after an in-flight tool has a durable
result, between model steps, or immediately before a terminal answer commits.
...
def _patch_runtime_class(runtime_cls: type[Any]) -> None:
    ...
    def steer(
        ...
        *,
        ...
        # the completed turn wins and the stale steer is rejected.
        with self._active_tokens_guard:
            ...
            if (
                ...
                if resolved_input_id in session.steering_ids:
                    return _receipt(

...
class LoomDesktopWindow(v9.LoomDesktopWindow):
    ...
    - no bounce/elastic easing, no decorative looping animation
    ...
    # v5 consumes these constants from self, so the old motion primitives are
    # automatically brought onto the same timing scale.
    ...
    def _build_ui(self) -> None:
        ...
        # Refine the inherited composer glow into the same restrained violet
        # language used by primary actions and selected navigation.
    ...
    def _install_v10_interactions(self) -> None:
        # v5 installs motion before v7 creates conversation-library controls;
        # calling again here picks up those later controls without duplicating
        # existing effects.
    ...
    def _render_activity(self) -> None:
        ...
        if (
            self._motion_enabled
            and count > previous
            and self.activity_view.isVisible()
        ):
            self._pulse_widget(
                self.activity_view,
                "runtime-event",
    ...
    def showEvent(self, event: Any) -> None:  # noqa: N802
        ...
        QTimer.singleShot(
            65,
            lambda: self._fade_in_widget(
                self.findChild(QWidget, "workspaceHeader"),
                "startup-header",
                start=0.62,
                duration=self.MOTION_REVEAL_MS,
            )
            if self.findChild(QWidget, "workspaceHeader") is not None
            else None,
        )
        QTimer.singleShot(
            95,

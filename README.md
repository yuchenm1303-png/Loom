...
## Reliability alignment (2026-09-08)

The runtime now uses one model/tool loop with explicit incomplete-response handling,
cancellable bounded model requests, binding-checked approval recovery, project
AGENTS.md loading, automatic budgeted compaction, durable steering, and journaled
event/snapshot commits. See [implementation and validation](docs/alignment-implementation-2026-09-08.md)
for compatibility details and the remaining Codex gaps, including the unsuccessful
MXC enforcement run on this development host.
...
- Windows: Microsoft MXC `wxc-exec`, with platform probe and policy construction; real enforcement requires a supported, prepared host
...
`compact_context_with_model` runs a dedicated no-tools model request over the archived prefix, accounts for its model usage, and commits the resulting summary only after the checkpoint archive is durable. Manual compaction rejects active turns; automatic compaction runs at safe model-step boundaries.
...
Search currently returns provider snippets and source URLs; Browser navigation is handled separately by the Browser / Computer Use layer and its URL, permission, and content-boundary controls.
